<template>
  ...
  <div class="flex pt-3 justify-center">
    <Concert :artist="artist" :start="start" :end="end" :location="location" :concertId="concertId" :id="id" />
  </div>
  <div class="flex flex-row m-20 gap-10 flex-wrap">
    <div class="seats ring-1 ring-black rounded-[100px] w-[70%] p-20 flex flex-row">
      <div class="flex flex-col justify-center w-full">
        <div class="flex justify-center">
          <div class="ring-2 ring-green-400 p-5 bg-green-200 w-80 py-4 text-center justify-center">
            <h2>Stage</h2>
          </div>
        </div>
        <div class="pt-12 flex flex-row">
          ...
          <div class="row-line flex flex-col gap-5">
            <div v-for="row in rows" :key="row" class="row-name gap-1.5 flex flex-row ml-20">
              <RowSeats :row="row" :unavailable="row.seats.unavailable" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="ring-1 ring-black w-80 p-5 flex flex-col">
      <div class="flex flex-col gap-2">
        ...
        <div v-if="chosenSeats" class="self-center">
          <p v-for="seat in chosenSeats" :key="seat">
            Row: Stall {{ seat[0] }}, Seat: {{ seat[1] }}
          </p>
        </div>
        <div class="items-end self-center">
          <button @click="buySeats" class="ring-1 ring-black self-end font-medium p-3 rounded-md w-40">
